 <!-- lp-crm -->
  <div id="idcrm_group" class="form-group">
	 ...
	 <p class="form-control-static"><a target="_blank" href="?page=help#lpcrm">Видеоинструкция по подключению (откроется в новой вкладке)</a></p></div></div>
		 ...
		 <div id="idcrm_group" class="form-group">
	 <label class="col-sm-3 control-label"  for="idcrm">Учетная запись LP-СRМ: </label><div class="col-sm-9"><input group="crmt" class="form-control" <? if ($crm!='lpcrm') echo("disabled"); ?> id="idcrm" type="text" required name="idcrm" value="<?=  $idcrm ?>" oninput="lpcrm_func(this.value)" placeholder="Учетная запись Вашей LP-СRМ - ВАША_УЧЕТНАЯ_ЗАПИСЬ.lp-crm.biz или ВАША_УЧЕТНАЯ_ЗАПИСЬ.lp-crm.top"></div></div>
 <div id="key_crm_group" class="form-group">
	 <label class="col-sm-3 control-label" for="key_crm">Ключ доступа: </label><div class="col-sm-9"><input group="crmt" class="form-control" <? if ($crm!='lpcrm') echo("disabled"); ?> id="key_crm" type="text" required name="key_crm" value="<?=  $key_crm ?>" placeholder="Ключ доступа к Вашей LP_CRM-системе."></div> </div>	
    ...
    <option <? if (($country_crm=="RU") OR ($country_script=="RU")) echo "selected"; ?> value="RU">Россия</option>
    <option <? if (($country_crm=="UA") OR ($country_script=="UA")) echo "selected"; ?> value="UA">Украина</option>
	<option <? if (($country_crm=="BY") OR ($country_script=="BY")) echo "selected"; ?> value="BY">Белорусь</option>
    <option <? if (($country_crm=="KZ")OR ($country_script=="KZ")) echo "selected"; ?> value="KZ">Казахстан</option>
      </select></div></div>	
